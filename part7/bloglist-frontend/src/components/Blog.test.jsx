...
describe("Blog title and author test", () => {
  ...
  test("title and author", async () => {
    ...
    expect(div).toHaveTextContent("Prueba Yo");
  });

  test("click view for url and likes", async () => {
    const component = render(<Blog blog={blog} likeBlog={mockUpdateBlog} />);

    const user = userEvent.setup();
    const button = screen.getByText("view");
    await user.click(button);

    expect(component.container).toHaveTextContent(".com/");

    expect(component.container).toHaveTextContent("1");
  });

  test("click button for url and likes", async () => {
    render(<Blog blog={blog} likeBlog={mockUpdateBlog} />);
